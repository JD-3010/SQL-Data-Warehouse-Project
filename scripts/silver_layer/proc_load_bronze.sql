...
BEGIN
	...
	BEGIN TRY
		...
		PRINT '-----------------------------------------------------------------------------------';
		-- TRUNCATING AND INSERTING FOR AVOIDING DUPLICATION OF DATA
		...
		SELECT prd_id,
			  ...
			  CASE UPPER(TRIM(prd_line))
				   ...
				   WHEN 'T' THEN 'Touring'
				   ELSE 'N/A'
			  END AS prd_line, -- Mapped Product Line codes to Descriptive Values
			  CAST(prd_start_dt AS DATE) AS prd_start_date,
			  CAST(
					LEAD(prd_start_dt) OVER (PARTITION BY prd_key ORDER BY prd_start_dt)-1 AS DATE)
					AS prd_end_dt -- Calculated the end date as one day before the next start date
		...
		PRINT '==================================================='
		PRINT '>> LOADING DATA INTO SILVER LAYER COMPLETED'
